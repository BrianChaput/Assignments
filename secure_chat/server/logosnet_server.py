...
with open("server-key-private.pem", "rb") as key_file:
    server_private_key = serialization.load_pem_private_key(
        key_file.read(),
        password=None,
    )

# Function to decrypt the symmetric key using server's private key
...
# Function to generate and encrypt a symmetric key for the client
def generate_and_encrypt_symmetric_key(client_public_key_path):
...
def main():
    ...
    while inputs:
	 ...
	 ###
                ...
                msg_status = LNP.recv(s, data)

                if msg_status == "MSG_CMPLT":

                    msg_id, msg = LNP.get_msg_from_queue(s, data)
	         ...
	         #no username yet, this message is a username
                        ...
                        if username_status == "USERNAME-ACCEPT":
                            LNP.send(s, msg, username_status)
                            usernames[s] = msg
                            del user_connect_time[s]
